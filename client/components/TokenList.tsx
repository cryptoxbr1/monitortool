      ...
      <table className="min-w-full text-sm">
        <thead className="bg-muted/50 text-muted-foreground">
          <tr>
            ...
            <th className="px-3 py-2 text-right font-medium hidden sm:table-cell">Uni 24h Vol</th>
            <th className="px-3 py-2 text-right font-medium hidden sm:table-cell">Cake 24h Vol</th>
            <th className="px-3 py-2 text-right font-medium hidden md:table-cell">Uni Liquidity</th>
            <th className="px-3 py-2 text-right font-medium hidden md:table-cell">Cake Liquidity</th>
          </tr>
        </thead>
        <tbody>
          {data.map((t) => {
              ...
              <tr key={t.tokenName} className="border-t">
                <td className="px-3 py-2">
                  <div className="flex items-center gap-3">
                    <div className="h-7 w-7 shrink-0 rounded-md bg-[hsl(var(--brand-600))]/10 text-[hsl(var(--brand-600))] grid place-items-center text-xs font-bold">{t.tokenName[0]}</div>
                    <div>
                ...
                <td className="px-3 py-2 text-right hidden sm:table-cell">{formatUsd(uni?.volume24hUsd ?? null)}</td>
                <td className="px-3 py-2 text-right hidden sm:table-cell">{formatUsd(cake?.volume24hUsd ?? null)}</td>
                <td className="px-3 py-2 text-right hidden md:table-cell">{formatUsd(uni?.liquidityUsd ?? null)}</td>
                <td className="px-3 py-2 text-right hidden md:table-cell">{formatUsd(cake?.liquidityUsd ?? null)}</td>
              </tr>
            );
          })}
        </tbody>
