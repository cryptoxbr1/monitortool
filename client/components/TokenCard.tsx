...
export default function TokenCard({ token }: { token: TokenRealtime }) {
  ...
  const uniHigher =
    uniPrice != null && cakePrice != null && uniPrice > cakePrice;
  const cakeHigher =
    uniPrice != null && cakePrice != null && cakePrice > uniPrice;

  const uniPriceClass = uniHigher
    ? "text-emerald-600"
    : cakeHigher
      ? "text-rose-600"
      : "text-foreground";
  const cakePriceClass = cakeHigher
    ? "text-emerald-600"
    : uniHigher
      ? "text-rose-600"
      : "text-foreground";
    ...
    >
      <div className="p-4 flex items-center justify-between bg-gradient-to-r from-[hsl(var(--brand-600))] to-[hsl(var(--brand-500))] text-white">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-lg bg-white/15 ring-1 ring-white/25 flex items-center justify-center font-bold">
            {token.tokenName[0]}
          </div>
          <div>
            <div className="text-lg font-semibold leading-tight">
              {token.tokenName} / WETH
            </div>
            <div className="text-xs opacity-80 truncate max-w-[220px]">
              {token.tokenAddress}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-6 text-right">
          <div>
            ...
            <div className={cn("text-xl font-bold", uniPriceClass)}>
              {formatUsd(uniPrice)}
            </div>
          </div>
          <div>
            <div className="text-xs opacity-90">PancakeSwap Price</div>
            <div className={cn("text-xl font-bold", cakePriceClass)}>
              {formatUsd(cakePrice)}
            </div>
          </div>
        </div>
      ...
      <div className="p-4 grid grid-cols-2 gap-4">
        <div>
          ...
          <div className="text-base font-medium">
            {formatUsd(uni?.volume24hUsd ?? null)}
          </div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">
            PancakeSwap 24h Volume
          </div>
          <div className="text-base font-medium">
            {formatUsd(cake?.volume24hUsd ?? null)}
          </div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">Uniswap Liquidity</div>
          <div className="text-base font-medium">
            {formatUsd(uni?.liquidityUsd ?? null)}
          </div>
        </div>
        <div>
          <div className="text-xs text-muted-foreground">
          ...
          <div className="text-base font-medium">
            {formatUsd(cake?.liquidityUsd ?? null)}
          </div>
        </div>
      </div>
    </div>
